	...
	<div class="formbody">
		...
		<div id="usual1" class="usual">
			...
			<div id="tab1" class="tabson">
				...
				<form action="outcome/informationUpdate" method="POST"
					onsubmit="return checkSubmit();">
					<input type="hidden" name="id" value="${outcome['id']}">
					<ul class="forminfo">
						<li><label>支出金额<b>*</b></label><input id="price" value="${outcome['price']}" onkeyup="value=value.replace(/[^\d.]/g,'')"
							name="price" type="text" class="dfinput" placeholder="请填写支出金额"
							style="width: 518px;" /></li>
						<li><label>支出时间<b>*</b></label><input id="come_date" value="${outcome['come_date']}"
						...
						<select class="select3" name="type_id" id="type_id">
										<c:forEach items="${typeeList }" var="typee">
											<c:if test="${outcome['typee']['id'] eq typee['id']}" var="s1">
						...
						<select class="select3" name="family_id" id="family_id">
										<c:forEach items="${familyList }" var="family">
											<c:if test="${outcome['family']['id'] eq family['id']}" var="s2">
