...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/outcomeAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("outcome/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="outcome/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${outcomeList.datas }" var="outcome" varStatus="vg">
						<tr>
							<td><c:out value="${outcome['family']['name']}"></c:out></td>
							<td><c:out value="${outcome['price']}"></c:out></td>
							<td><c:out value="${outcome['typee']['name']}"></c:out></td>
							<td><c:out value="${outcome['come_date']}"></c:out></td>
							<td><c:out value="${outcome['createtime']}"></c:out></td>
							<td><a href="javascript:;" onclick="update(${outcome['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${outcome['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="outcome/informationLoad?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="outcome/informationLoad?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="outcome/informationLoad?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="outcome/informationLoad?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="outcome/informationLoad?pageNow=${count }">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="outcome/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
