...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/countAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("count/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="count/countOutcome" method="POST"
		onsubmit="return check();">
	<div class="formbody">
			<div id="usual1" class="usual">
				<ul class="seachform1">
					<li><label>成员名称</label><input type="text" class="scinput1"
						name="searchName" value="${countt['searchName'] }"></li>
					<li><input type="text" class="scinput1"
						name="start_time" id="start" value="${countt['start_time'] }" placeholder="开始时间"></li>
					<li><input type="text" class="scinput1"
						name="end_time" id="end" value="${countt['end_time'] }" placeholder="结束时间"></li>
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${countList.datas }" var="count" varStatus="vg">
						<tr>
							<td><c:out value="${count['name']}"></c:out></td>
							<td><c:out value="${count['priceSum']}"></c:out></td>
						</tr>
					</c:forEach>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="count/countOutcome?pageNow=${pager.pageNow }&searchName=${countt['searchName']}&start_time=${countt['start_time']}&end_time=${countt['end_time']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="count/countOutcome?pageNow=${pager.pageNow-1 }&searchName=${countt['searchName']}&start_time=${countt['start_time']}&end_time=${countt['end_time']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="count/countOutcome?pageNow=${vs.count }&searchName=${countt['searchName']}&start_time=${countt['start_time']}&end_time=${countt['end_time']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="count/countOutcome?pageNow=${count }&searchName=${countt['searchName']}&start_time=${countt['start_time']}&end_time=${countt['end_time']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="count/countOutcome?pageNow=${count }&searchName=${countt['searchName']}&start_time=${countt['start_time']}&end_time=${countt['end_time']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="count/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
