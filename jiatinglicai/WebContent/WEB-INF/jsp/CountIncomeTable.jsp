...
<body>
	...
	<form action="count/countIncome" method="POST"
		onsubmit="return check();">
	<div class="formbody">
			<div id="usual1" class="usual">
				<ul class="seachform1">
					<li><label>成员名称</label><input type="text" class="scinput1"
						name="searchName" value="${count['searchName'] }"></li>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="count/countIncome?pageNow=${pager.pageNow }&searchName=${countt['searchName']}&start_time=${countt['start_time']}&end_time=${countt['end_time']}">
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="count/countIncome?pageNow=${vs.count }&searchName=${countt['searchName']}&start_time=${countt['start_time']}&end_time=${countt['end_time']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="count/countIncome?pageNow=${count }&searchName=${countt['searchName']}&start_time=${countt['start_time']}&end_time=${countt['end_time']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="count/countIncome?pageNow=${count }&searchName=${countt['searchName']}&start_time=${countt['start_time']}&end_time=${countt['end_time']}">${count}</a></li>
					...
					<li class="paginItem"><c:if
							...
							<a
									href="count/countIncome?pageNow=${pager.pageNow+1 }&searchName=${countt['searchName']}&start_time=${countt['start_time']}&end_time=${countt['end_time']}">
								<span class="pagenxt"></span>
							</a>
						</c:if></li>
				</ul>
